...
class LocalRecordsConfig(AppConfig):
		...
		if len(current_records) > 0:
			...
			if race_time < current_record.score:
				...
				if new_index < previous_index:
					message = '$z$s> $fff{}$z$s$0f3 gained the $fff{}.$0f3 Local Record, with a time of $fff{}$0f3 ($fff{}.$0f3 $fff-{}$0f3).'.format(player.nickname, new_index, times.format_time(race_time), previous_index, times.format_time((previous_time - race_time)))
				else:
					message = '$z$s> $fff{}$z$s$0f3 improved the $fff{}.$0f3 Local Record, with a time of $fff{}$0f3 ($fff-{}$0f3).'.format(player.nickname, new_index, times.format_time(race_time), times.format_time((previous_time - race_time)))

				await self.instance.gbx.execute('ChatSendServerMessage', message)

			elif race_time == current_record.score:
				message = '$z$s> $fff{}$z$s$0f3 equalled the $fff{}.$0f3 Local Record, with a time of $fff{}$0f3.'.format(player.nickname, previous_index, times.format_time(race_time))
				await self.instance.gbx.execute('ChatSendServerMessage', message)
			...
			message = '$z$s> $fff{}$z$s$0f3 drove the $fff{}.$0f3 Local Record, with a time of $fff{}$0f3.'.format(player.nickname, new_index, times.format_time(race_time))
			await self.instance.gbx.execute('ChatSendServerMessage', message)
	...
	async def chat_personal_record(self, player):
		...
		if len(record) > 0:
			message = '$z$s> $0f3You currently hold the $fff{}.$0f3 Local Record: $fff{}'.format(self.current_records.index(record[0]) + 1, times.format_time(record[0].score))
