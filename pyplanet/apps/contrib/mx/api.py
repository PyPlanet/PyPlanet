...
class MXApi:
	...
	async def pack_info(self, id, token):
		url = 'https://api.mania-exchange.com/tm/mappack/{id}/tracks/?={secret}'.format(
			site=self.site,
			id=id,
			secret=token
		)
	...
	async def download_pack(self, mxpack_id, secret):
		url = 'https://{site}.mania-exchange.com/mappack/download/{id}?{param}'.format(
			site=self.site,
			id=mxpack_id,
			param=secret
		)
		...
		response = await self.session.get(url, params=params)
		if response.status == 404:
			raise MXMapNotFound('Map pack has not been found!')
		if response.status == 302:
			raise MXInvalidResponse(
				'Map author has declined download of the map pack. Status code: {}'.format(response.status))
		if response.status < 200 or response.status > 399:
			raise MXInvalidResponse('Got invalid response status from ManiaExchange: {}'.format(response.status))
		return response
