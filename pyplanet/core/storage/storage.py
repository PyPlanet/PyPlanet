...
class Storage(StorageInterface):
	...
	def __init__(self, instance, driver: StorageDriver):
		"""
		...
		self._tmp_root = os.path.join(settings.PYPLANET_TMP_PATH)
		self._tmp_driver = os.path.join(self._tmp_root, )

	@classmethod
	def create_from_settings(cls, instance, config):
		driver_path, _, driver_cls_name = config.PYPLANET_STORAGE_DRIVER.rpartition('.')
		driver_cls = getattr(importlib.import_module(driver_path), driver_cls_name)
		driver = driver_cls(instance, config)
